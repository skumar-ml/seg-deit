...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, frequency=False):
        super().__init__()
        img_size = to_2tuple(img_size)
        self.patch_size = to_2tuple(patch_size)
        num_patches = (img_size[1] // self.patch_size[1]) * (img_size[0] // self.patch_size[0])
        ...
        if self.frequency:
            self.proj = nn.Linear(patch_size*int(patch_size/2 + 1)*3*2, embed_dim)
    ...
    def forward(self, x):
        ...
        if self.frequency:
            # Split into patches, reshape into (B, C, P, H, W) | P = number of patches, H = W = patch_size
            x = self.unfold(x)
            x = x.reshape(B, C, self.patch_size[0], self.patch_size[1], -1)
            x = x.permute(0, -1, 1, 2, 3)

            # Take FFT and flatten for projection
            x = torch.fft.rfft2(x)
            x = torch.stack((x.angle(), x.abs()), dim=-1)
...
class SegVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if bag_of_words:
            self.proj_freq = nn.Linear(n_points, embed_dim)
        else:
